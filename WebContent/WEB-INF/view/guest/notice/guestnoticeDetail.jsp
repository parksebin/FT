...
<%
	...
	System.out.println(CmmUtil.nvl(noticeDto.getNotice_seq()));
...
<script type="text/javascript">
	...
	*/
	...
	function doAction(gubun) {
		...
		if (gubun == "U"){
			f.action = "guestnoticeupdateForm.do";
			f.submit();
		} else if (gubun == "D") {
			if (confirm("삭제하시겠습니까?")) {
	...
	function doCommList(){
		...
		xmlHttpPost("/notice/guestCommentList.do", "notice_seq=<%=CmmUtil.nvl(noticeDto.getNotice_seq())%>" , "viewResult");
	...
	function viewResultMoreView(result) {
		...
		for (var i=0;i<jsonObj.list.length;i++){
			if(login == jsonObj.list[i].USER_NO){	
				...
				tag +="  	<input type=\"button\" value=\"수정\" onclick=\"doCommentUpdateView('"+ jsonObj.list[i].COMT_SEQ +"', '"+ jsonObj.list[i].CONTENTS +"')\"  class=\"btn_modify\">";
				tag +="   	<input type=\"button\" value=\"삭제\" onclick=\"doCommentDelete('"+ jsonObj.list[i].COMT_SEQ +"')\"  class=\"btn_delete\">";
				tag +="		<input type=\"button\" title=\"목록\" value=\"목록\" onclick=\"location.href='guestEventList.do'\" class=\"btn_global\">";			
				tag +="  </div>";
				tag +="</div>";
				tag +="</li>";
	...
	function saveComment() {
		...
		if (commentSaveType=="A"){
			var actionUrl = "/notice/guestInsertComment.do"; //호출 url 
		...
		//댓글 수정			
			...
			var actionUrl = "/notice/guestUpdateComment.do"; //호출 url 
	...
	function doCommentDelete(com_seq){
		...
		if(confirm("삭제하시겠습니까?")){
			var actionUrl = "/notice/guestDeleteComment.do"; //호출 url 
	...
	function moreViewComment(){
		...
		var actionUrl = "/notice/guestCommentList.do"; //호출 url 
		var submitParameter  ="notice_seq=<%=CmmUtil.nvl(noticeDto.getNotice_seq())%>";
...
<body onload="doOnload()">
				...
				<div class="content_wrapper">
					...
					<header>
						<div class='view_title'>
							<%=CmmUtil.nvl(noticeDto.getTitle())%>
						</div>
						<span class='view_info'> <span> <%=CmmUtil.nvl(noticeDto.getUser_name())%></span><br />
							<%=CmmUtil.nvl(noticeDto.getReg_dt())%>&nbsp;&nbsp;|&nbsp;&nbsp;조회 <%=CmmUtil.nvl(noticeDto.getRead_cnt())%>
					...
					<div class="section">
						...
						<div class="view_detail">
							<img src="<%=CmmUtil.nvl(noticeDto.getFile_name())%>" /><br />
							<%=CmmUtil.nvl(noticeDto.getContents())%>
						</div>

						<div class="view_modify btn_wrap">
							<input type="button" title="목록" value="목록"
								onclick="location.href='guestnoticeList.do'" class="btn_global">
			...
			<div class="content view" style="display: block;">
				<div class="content_wrapper">
					<div class="reply_head">
						댓글 <span id="commentListCnt"> 0 </span>
					</div>
 ...
 %> <textarea name="contents" placeholder="로그인 후 댓글 등록이 가능합니다"  readonly></textarea>
								...
								<div id="btnCommentArea" class="view_modify btn_wrap">
									...
									%>
									<input type="button" id="btnSave" value="댓글 등록"
										onclick="saveComment()" class="btn_confirm">
						...
						<input id="btnMoreView" type="button" title="더보기"
							onclick="moreViewComment()" value="더보기" class="btn_white" />
					</ul>
