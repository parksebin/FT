...
<script type="text/javascript">
	...
	function doCommList() {
		...
		xmlHttpPost("/event/guestEventIndex.do", searchParameter, "viewResult");
	...
	function viewResult(result) {
		...
		for (var i = 0; i < jsonObj.list.length; i++) {
			...
			tag += "    <a href=\"guestEventDetail.do?ent_seq="+ jsonObj.list[i].ENT_SEQ + "\">";
	...
	function viewResultMoreView(result) {
		...
		for (var i = 0; i < jsonObj.list.length; i++) {
			...
			tag += "    <a href=\"guestEventDetail.do?ent_seq="+ jsonObj.list[i].ENT_SEQ + "\">";
			tag += "     " + jsonObj.list[i].TITLE + "</span></a><br/>";
			tag += "      <span class='desc'>" + jsonObj.list[i].USER_NAME+ "&nbsp;|&nbsp;";
			tag += "     " + jsonObj.list[i].REG_DT + "|";
			tag += "      조회" + jsonObj.list[i].READ_CNT + "</span></span>";
			tag += "     <div class=\"icon icon-comment\"><span>";
			if(jsonObj.list[i].CNT==""){
	...
	function moreViewComment() {
		...
		var actionUrl = "/event/guestEventIndex.do"; //호출 url 
	...
	//등록화면 이동
	function doReg() {
		location.href = "/event/guestEventForm.do";
	...
	<div class="wrapper">
		...
		<div class="content" style='display: block;'>
			<div class="content_wrapper">
				...
				<form id="search" name="search" method="post" action="/event/guestEventList.do">
					<!-- group -->
					<div class="group">
						<div id="search">
					...
					<div class='board_desc'>
						...
						<!-- 							onclick="checkboxChange(this.form, this)" /><span>전체선택</span> -->
						<h3><strong>방문자 이상 읽기가 가능한 게시판입니다.</strong></h3>
					</div>
					<ul class='member'>
					</ul>
					<!-- list -->
