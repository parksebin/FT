...
<% 
	...
	//서버에서 가져온 유저 넘버
	String login_no = CmmUtil.nvl(communityDto.getUser_no());
	
	System.out.println(login_no);

	//jsp에서 사용할 세션값
	String login2 = (String)session.getAttribute("USER_NO");
	if (login2 == null){
		login2 = "";
	}
...
<script type="text/javascript">
	...
	*/
	...
	function doAction(gubun) {
		...
		if (gubun == "U"){
			f.action = "guestcommunityupdateForm.do";
			f.submit();
		} else if (gubun == "D") {
			if (confirm("삭제하시겠습니까?")) {
	...
	//댓글 리스트 조회하기
	function doCommList(){
		xmlHttpPost("/community/guestCommentList.do", "cmt_seq=<%=CmmUtil.nvl(communityDto.getCmt_seq())%>" , "viewResult");
	...
	function saveComment() {
		...
		if (commentSaveType=="A"){
			var actionUrl = "/community/guestInsertComment.do"; //호출 url 
		...
		//댓글 수정			
			...
			var actionUrl = "/community/guestUpdateComment.do"; //호출 url 
	...
	function doCommentDelete(com_seq){
		...
		if(confirm("삭제하시겠습니까?")){
			var actionUrl = "/community/guestDeleteComment.do"; //호출 url 
	...
	function moreViewComment(){
		...
		var actionUrl = "/community/guestCommentList.do"; //호출 url 
			...
			<div class="section">
                ...
                <div class="view_modify btn_wrap">
                    ...
                    <input type="button" title="삭제" value="삭제" onclick="doAction('D')" class="btn_tri">
      				<input type="button" title="목록" value="목록" onclick="location.href='guestcommunityList.do'"class="btn_tri">
                	<%}else{ %>
                
                <input type="button" title="목록" value="목록" onclick="location.href='guestcommunityList.do'"class="btn_global">
