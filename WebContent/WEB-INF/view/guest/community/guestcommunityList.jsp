...
<script type="text/javascript">
	...
	//게시글 리스트 조회하기
	function doCommList() {
		xmlHttpPost("/community/guestCommunityIndex.do", searchParameter, "viewResult");
	...
	function viewResult(result) {
		...
		for (var i = 0; i < jsonObj.list.length; i++) {
			...
			tag += "    <a href=\"guestcommunityDetail.do?cmt_seq="+ jsonObj.list[i].CMT_SEQ + "\">";
	...
	function moreViewComment() {
		...
		var actionUrl = "/community/guestCommunityIndex.do"; //호출 url 
	...
	//등록화면 이동
	function doReg() {
		location.href = "/community/guestcommunityForm.do";
		...
		<div class="content" style='display: block;'>
			<div class="content_wrapper">
				...
				<form id="search" name="search" method="post"
					action="/community/guestcommunityList.do">
					<!-- group -->
					<div class="group">
						<div id="search">
				...
				<form id="f" name="f" action="guestdeleteCheck.do" method="post">
					<ul class='member'>
					</ul>
					<!-- list -->
					...
					<%  if (session.getAttribute("USER_NO") == null || session.getAttribute("USER_NAME").equals("")) {%>
						...
						<%}else{ %>
						<input type="button" title="등록" value="등록" onclick="doReg()"class="btn_global"> 
						<%} %>
