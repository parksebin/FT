...
<script type="text/javascript">
	...
	*/
	...
	function doAction(gubun) {
		...
		if (gubun == "U"){
			f.action = "busieventupdateForm.do";
			f.submit();
		} else if (gubun == "D") {
			if (confirm("삭제하시겠습니까?")) {
	...
	//댓글 리스트 조회하기
	function doCommList(){
		xmlHttpPost("/event/busiCommentList.do", "ent_seq=<%=CmmUtil.nvl(eventDto.getEnt_seq())%>" , "viewResult");
	...
	function viewResultMoreView(result) {
		...
		$("#replyListArea").html(tag); //class='date' 회색
	...
	function saveComment() {
		...
		if (commentSaveType=="A"){
			var actionUrl = "/event/busiInsertComment.do"; //호출 url 
			var submitParameter ="ent_seq=<%=CmmUtil.nvl(eventDto.getEnt_seq())%>&contents=" + f.contents.value; 
		...
		//댓글 수정			
			...
			var actionUrl = "/event/busiUpdateComment.do"; //호출 url 
			var submitParameter ="ent_seq=<%=CmmUtil.nvl(eventDto.getEnt_seq())%>&com_seq="+ f.com_seq.value +"&contents=" + f.contents.value; 
	...
	function doCommentDelete(com_seq){
		...
		if(confirm("삭제하시겠습니까?")){
			var actionUrl = "/event/busiDeleteComment.do"; //호출 url 
			var submitParameter ="ent_seq=<%=CmmUtil.nvl(eventDto.getEnt_seq())%>&com_seq=" + com_seq; 
	...
	function moreViewComment(){
		...
		var actionUrl = "/event/busiCommentList.do"; //호출 url 
		var submitParameter  ="ent_seq=<%=CmmUtil.nvl(eventDto.getEnt_seq())%>";
...
Kakao.init('0e8a2f28c36abba882c9b2238718b483');
...
Kakao.Link.sendTalkLink({
  label: '라오푸드트럭',
  image: {
	  src: 'http://image.shutterstock.com/z/stock-vector-logo-of-food-truck-428763541.jpg',
  ...
  webButton: {
    text: '라오푸드트럭',
    url: 'https://www.instagram.com/laofoodtruck/' // 앱 설정의 웹 플랫폼에 등록한 도메인의 URL이어야 합니다.
  }
});
}
...
// 카카오스토리 공유하기
  function shareStory() {
    Kakao.Story.share({
      url: 'https://www.instagram.com/laofoodtruck/',
      text: '공유제목'
    });
  }
...
<%@include file="/busiinc/appmenu.jsp"%>
    ...
    <form name="form1" id="form1" method="post">
  	<input type="hidden" name="ent_seq" value="<%=CmmUtil.nvl(eventDto.getEnt_seq())%>">
        ...
        <div class="content_wrapper">
        	...
        	<header>
                <div class='view_title'>
                   <%=CmmUtil.nvl(eventDto.getTitle())%>
                </div>
                <span class='view_info'>
                    <span>   <%=CmmUtil.nvl(eventDto.getUser_name())%></span><br/>
                   <%=CmmUtil.nvl(eventDto.getReg_dt())%> | 조회 <%=CmmUtil.nvl(eventDto.getRead_cnt())%>
			...
			<div class="section3">
				...
				<img src="<%=CmmUtil.nvl(eventDto.getFile_name())%>" /><br/>
				<%=CmmUtil.nvl(eventDto.getContents())%>
				</div>
              	<div class="view_modify btn_wrap">
            
              		 <input type="button" title="수정" value="수정" onclick="doAction('U')" class="btn_tri1">
              		 <input type="button" title="삭제" value="삭제" onclick="doAction('D')" class="btn_tri1">
      		  		 <input type="button" title="목록" value="목록" onclick="location.href='busieventList.do'"class="btn_tri1">
  				<div class='clear'></div>
                
        		<div class='sns2'>
					<a href="javascript:toSNS('twitter','','https://www.instagram.com/laofoodtruck/')" title="트위터로 가져가기"><img src="/img/twiter.JPG"></a>
					<a href="javascript:toSNS('facebook','','https://www.instagram.com/laofoodtruck/')" title="페이스북으로 가져가기"><img src="/img/face.JPG"></a>
         			<a href="javascript:sendKakaoTalk();" title="카카오톡으로 가져가기"><img src="/img/Kakao.JPG"></a>
         			<a href="javascript:toSNS('band','','https://www.instagram.com/laofoodtruck/')" title="밴드로 가져가기"><img src="/img/band.JPG"></a>
         			<a href="javascript:shareStory();" title="카카오스토리로 가져가기"><img src="/img/kakastory.JPG"></a>
            	</div>            
           </div>       
        </div>
	</div>
