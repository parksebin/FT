...
<script type="text/javascript">
	...
	function doCommList() {
		...
		xmlHttpPost("/event/busiEventIndex.do", searchParameter, "viewResult");
	...
	function viewResult(result) {
		...
		for (var i = 0; i < jsonObj.list.length; i++) {
			...
			//객체가 널이 아니라면.....	
			tag += "<li>";
			tag += "	<input type='checkbox' name='del_chk' value=\"" + jsonObj.list[i].ENT_SEQ + "\">";
			tag += "   <span class='w70'><span class='title ep'>";
			tag += "    <a href=\"busieventDetail.do?ent_seq="+ jsonObj.list[i].ENT_SEQ + "\">";
	...
	function moreViewComment() {
		...
		var actionUrl = "/event/busiEventIndex.do"; //호출 url 
	...
	//등록화면 이동
	function doReg() {
		location.href = "/event/busieventForm.do";
					...
					<ul class='member'>
						<li class='list_head'><input type="checkbox" name="all"
							onclick="checkboxChange(this.form, this)" /> <span>전체선택</span> <!-- 		<a href='#' class="icon-trash">
								...
								<div class="trash-container" onclick="userdel_check()"></div>
						</a> --></li>
					</ul>
					<!-- list -->
