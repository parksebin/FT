...
<script type="text/javascript">
	...
	//게시글 리스트 조회하기
	function doCommList() {
		xmlHttpPost("/community/busiCommunityIndex.do", searchParameter, "viewResult");
	...
	function viewResult(result) {
		...
		for (var i = 0; i < jsonObj.list.length; i++) {
			...
			//객체가 널이 아니라면.....	
			tag += "<li>";
			tag += "	<input type='checkbox' name='del_chk' value=\"" + jsonObj.list[i].CMT_SEQ + "\">";
			tag += "   <span class='w70'><span class='title ep'>";
			tag += "    <a href=\"busicommunityDetail.do?cmt_seq="+ jsonObj.list[i].CMT_SEQ + "\">";
	...
	function moreViewComment() {
		...
		var actionUrl = "/community/busiCommunityIndex.do"; //호출 url 
	...
	//등록화면 이동
	function doReg() {
		location.href = "/community/busicommunityForm.do";
		...
		<div class="content" style='display: block;'>
			<div class="content_wrapper">
				...
				<form id="search" name="search" method="post"
					action="/community/busicommunityList.do">
					<!-- group -->
					<div class="group">
						<div id="search">
				...
				<form id="f" name="f" action="busideleteCheck.do" method="post">
					<ul class='member'>
						<li class='list_head'><input type="checkbox" name="all"
							onclick="checkboxChange(this.form, this)" /> <span>전체선택</span> <!-- 		<a href='#' class="icon-trash">
...
onclick="userdel_check()"></div>
					...
					<% 	if(session.getAttribute("USER_NO")==null){%> 
										...
										<%}else{ %>
						<input type="button" title="등록" value="등록" onclick="doReg()"
							class="btn_half left"> <input type="button" title="목록"
							value="삭제" onclick="userdel_check()" class="btn_half left">
					</div>
						<%} %>
