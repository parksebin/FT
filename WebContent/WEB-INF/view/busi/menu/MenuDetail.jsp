...
function doAction(gubun) {
	...
	if (gubun == "U"){
		f.action = "busimenuupdateForm.do";
		f.submit();
	} else if (gubun == "D") {
		if (confirm("삭제하시겠습니까?")) {
...
//댓글 리스트 조회하기
function doCommList(){
	xmlHttpPost("/menu/busiCommentList.do", "menu_seq=<%=CmmUtil.nvl(menuinfoDto.getMenu_seq())%>", "viewResult");
...
function saveComment() {
	...
	if (commentSaveType=="A"){
		var actionUrl = "/menu/busiInsertComment.do"; //호출 url 
	...
	//댓글 수정			
		...
		var actionUrl = "/menu/busiUpdateComment.do"; //호출 url 
...
function doCommentDelete(com_seq){
	...
	if(confirm("삭제하시겠습니까?")){
		var actionUrl = "/menu/busiDeleteComment.do"; //호출 url 
...
function moreViewComment(){
	...
	var actionUrl = "/menu/busiCommentList.do"; //호출 url 
   ...
   <%@include file="/busiinc/appheader.jsp"%>
    ...
    <div class="content" style="display:block;">
        <div class="content_wrapper">
        	<header><span><%=CmmUtil.nvl(menuinfoDto.getMenu_name()) %></span></header>
			<div class="section">
               <div class='food_main'><img src="<%=CmmUtil.nvl(menuinfoDto.getFile_name())%>" /></div>
                ...
                <div class='wrapper50'>
                    ...
                    <input type="button" title="목록" value="목록" class="btn_tri" onclick="location.href='/menu/busiMenuList.do'">
                </div>
				</form>
