...
<script type="text/javascript">
	...
	function doCommList() {
		...
		xmlHttpPost("/notice/busiNoticeIndex.do", searchParameter, "viewResult");
	...
	function viewResult(result) {
		...
		for (var i = 0; i < jsonObj.list.length; i++) {
			...
			//객체가 널이 아니라면.....	
			tag += "<li>";
			tag += "	<input type='checkbox' name='del_chk' value=\"" + jsonObj.list[i].NOTICE_SEQ + "\">";
			tag += "   <span class='w70'><span class='title ep'>";
			tag += "    <a href=\"businoticeDetail.do?notice_seq="+ jsonObj.list[i].NOTICE_SEQ + "\">";
	...
	function moreViewComment() {
		...
		var actionUrl = "/notice/busiNoticeIndex.do"; //호출 url 
	...
	//등록화면 이동
	function doReg() {
		location.href = "/notice/businoticeForm.do";
		...
		<div class="content" style='display: block;'>
			<div class="content_wrapper">
				...
				<form id="search" name="search" method="post"
					action="/notice/businoticeList.do">
					<!-- group -->
					<div class="group">
						<div id="search">
...
name="searchText" placeholder="제목 / 내용"
				...
				<form id="f" name="f" action="busideleteCheck.do" method="post">
					<ul class='member'>
						<li class='list_head'><input type="checkbox" name="all"
							onclick="checkboxChange(this.form, this)" /> 

<span>전체선택</span> <!-- 		<a href='#' class="icon-trash">
								<div class="trash-lid"></div>
								<div class="trash-container" 

onclick="userdel_check()"></div>
						</a> --></li>
					</ul>
					<!-- list -->
					...
					<div class="content wrapper50">
						...
						<%}else{ %>
						<input type="button" title="등록" value="등록" onclick="doReg()" class="btn_half left"> 
							<input type="button" title="삭제"value="삭제" onclick="userdel_check()" class="btn_half left">
					</div>
					<%} %>
