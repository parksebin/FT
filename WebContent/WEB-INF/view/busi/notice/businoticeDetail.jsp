...
<script type="text/javascript">
	...
	*/
	...
	function doAction(gubun) {
		...
		if (gubun == "U"){
			f.action = "businoticeupdateForm.do";
			f.submit();
		} else if (gubun == "D") {
			if (confirm("삭제하시겠습니까?")) {
	...
	//댓글 리스트 조회하기
	function doCommList(){
		xmlHttpPost("/notice/busiCommentList.do", "notice_seq=<%=CmmUtil.nvl(noticeDto.getNotice_seq())%>" , "viewResult");
	...
	function saveComment() {
		...
		if (commentSaveType=="A"){
			var actionUrl = "/notice/busiInsertComment.do"; //호출 url 
		...
		//댓글 수정			
			...
			var actionUrl = "/notice/busiUpdateComment.do"; //호출 url 
	...
	function doCommentDelete(com_seq){
		...
		if(confirm("삭제하시겠습니까?")){
			var actionUrl = "/notice/busiDeleteComment.do"; //호출 url 
	...
	function moreViewComment(){
		...
		var actionUrl = "/notice/busiCommentList.do"; //호출 url 
...
<%@include file="/busiinc/appmenu.jsp"%>
    ...
    <form name="form1" id="form1" method="post">
  	<input type="hidden" name="notice_seq" value="<%=CmmUtil.nvl(noticeDto.getNotice_seq())%>">
        ...
        <div class="content_wrapper">
        	...
        	<header>
                ...
                <span class='view_info'>
                    <span>   <%=CmmUtil.nvl(noticeDto.getUser_name())%></span><br/>
			...
			<div class="section">
                ...
                <div class="view_modify btn_wrap">
                    ...
                    <input type="button" title="삭제" value="삭제" onclick="doAction('D')" class="btn_tri">
      				<input type="button" title="목록" value="목록" onclick="location.href='businoticeList.do'"class="btn_tri">
                </div>
            </div>
        </div>
    </div>
